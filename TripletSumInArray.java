...
MediumAccuracy: 35.0%Submissions: 237K+Points: 4
...
class Solution
{
    ...
    { 
       ...
       Arrays.sort(A);      //first sort the array to ascending order
        for( int i=0 ; i<n ; i++ )
        {
            if( i==0 || ( i>0 && A[i]!=A[i-1]) )
            {
                //some of the array element are same so we apply the operation only ones 
                ...
                while( low<high )
                {
                    if( A[low]+A[high]==sum )
                    {
                        //if find the triplet sum then return true
                        return true;
                    }
                    else if( A[low]+A[high]<sum )
                    {
                        //if sum is greter low pointer increase beacuse array is ascending order
                        low++;
                    }
                    else
                    {
        ...
        //if not found any triplet return false
        return false;
    }
}
