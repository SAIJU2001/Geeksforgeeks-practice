...
HardAccuracy: 64.8%Submissions: 60K+Points: 8
...
class Clone 
{
    ...
    Node copyList(Node head) 
    {
        ...
        //make copy of each node and link them together\\
        Node front=head;
        Node item=head;
        while( item!=null )
        {
        ...
        //assign random pointer to the copy node\\
        item=head;
        while( item!=null )
        {
            if( item.arb!=null )
            {
                item.next.arb=item.arb.next;
            }
            item=item.next.next;
        }
        
        //restore the original and copy list\\
